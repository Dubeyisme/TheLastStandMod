...
GOLEM_INIT_LIST = {"npc_dota_neutral_mud_golem","npc_dota_neutral_rock_golem","npc_dota_neutral_granite_golem"}

GOLEM_SPAWN_LIST = {"npc_dota_neutral_mud_golem_split","npc_dota_neutral_mud_golem","npc_dota_neutral_rock_golem"}
...
function UnitAbilities:CheckActionOnDeath(died)
	...
	for i=1, #GOLEM_INIT_LIST do
		if(died:GetUnitName()==GOLEM_INIT_LIST[i])then
...
function UnitAbilities:GolemSpawner(died, num)
	...
	for i=1,2 do
		if(died:GetTeam()==DOTA_TEAM_BADGUYS)then
			unit = TheLastStand:CreateWaveUnitAtPlace(GOLEM_SPAWN_LIST[num], died:GetOrigin(), nil, died:GetTeam(), true, false)
		else
  			unit = CreateUnitByName(GOLEM_SPAWN_LIST[num], died:GetOrigin(), true, died:GetOwner(), died:GetOwner(), died:GetTeam(), false)
